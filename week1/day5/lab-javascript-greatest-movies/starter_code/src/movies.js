...
function howManyMovies(arr) {
  ...
  if (arr.length === 0) {
    ...
    return arr.filter(a => a.director == "Steven Spielberg" && a.genre.indexOf("Drama") !== -1).length;
  }
}
...
function orderAlphabetically(arrNotMutated) {
  ...
  if (arrTemp.length == 0) {
    ...
    return arrTemp.map(v => v.title).sort((a, b) => a.localeCompare(b)).slice(0, 20);
...
function dramaMoviesRate(arr) {
  if (arr.length == 0 || arr.filter(v => v.genre == "Drama").length == 0) {
    ...
    var sum = 0;
    var count = 0;
    return ratesAverage(arr.filter(v => v.genre == "Drama"))
